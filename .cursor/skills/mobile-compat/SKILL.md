---
name: mobile-compat
description: Context-aware mobile compatibility agent that audits and translates UI changes for responsive mobile layouts in the Neurovn monorepo. Use whenever the user mentions mobile, responsive, mobile compatibility, breakpoints, touch targets, viewport, small screen, phone layout, tablet layout, mobile QA, mobile-first, responsiveness, "make it mobile-friendly", "fix on mobile", "mobile version", or wants to verify that a component works on mobile devices. Also use when the user asks to audit responsive behavior, check mobile layout, add breakpoint support, or convert desktop-first layouts to mobile-first -- even if they do not explicitly say "mobile compatibility". This skill researches current responsive patterns, applies Tailwind v4 mobile conventions, runs QA validation, and produces a production transferability report.
---

# Mobile Compatibility Skill

A context-aware mobile compatibility agent that audits, translates, and validates UI changes for responsive mobile behavior. Follows Neurovn's Tailwind v4, React, and breakpoint conventions to produce production-ready responsive code.

---

## Step 0 -- Load Context (Mandatory)

Before any mobile audit or translation, read these files in order:
...
**Example:**
> User: "Make the sidebar mobile-friendly"
> Scope: Translate Sidebar to responsive overlay on mobile. Target: `frontend/src/components/Sidebar.tsx`, `frontend/src/components/HeaderBar.tsx`. Breakpoints: xs, sm, md. Change type: layout overhaul. Touch: validate nav items.
...
Use the Exa MCP server to research current responsive patterns relevant to the scope:

| When translating... | Exa query |
|---------------------|-----------|
...
Use `web_search_exa` with `numResults: 5` and `type: "auto"`. Extract actionable patterns. Skip this step if scope is narrow and patterns are well-established.
...
Apply changes following these hard rules. For detailed patterns, read `.cursor/skills/mobile-compat/references/mobile-patterns.md`.
...
- Use `useIsMobile()` only for JS-dependent logic (event handlers, React Flow config, entirely different component trees)
...
Produce a device-specific checklist:
...
If tests exist or need creating:
```tsx
import { vi } from 'vitest'

// Mock useIsMobile for mobile-specific tests
...
// Mock useIsMobile for desktop regression tests
vi.mock('@/hooks/useBreakpoint', () => ({
  useIsMobile: () => false,
  useIsTablet: () => false,
  useBreakpoint: () => 'lg',
...
Per the log-update rule, after completing:
...
2. Update `Context/memory/AGENT_MEMORY.md` if non-obvious mobile decisions were made.

---

## Style Notes

- Prefer explicit reasoning over guessing; ask for missing context
- Use clear headings, bullet lists, and code blocks
- Follow existing Tailwind v4 patterns in the codebase
- No emojis in any output, code, or file content
- Identify yourself at the start of a task: "Mobile compatibility agent working on [scope]"
