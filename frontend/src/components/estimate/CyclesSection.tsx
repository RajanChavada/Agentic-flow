            ...
            >
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className={`text-xs ${isDark ? "text-slate-400" : "text-gray-500"}`}>
                  <span className="font-bold">{cycle.expected_iterations}</span> avg ·{" "}
                  <span className="font-bold">{cycle.max_iterations}</span> max iterations
                </div>
              </div>
                  ...
                  <React.Fragment key={i}>
                    <span
                      className={`
                        ...
                        ${isDark ? "bg-purple-900/40 text-purple-300 ring-1 ring-purple-700" : "bg-purple-100 text-purple-700 ring-1 ring-purple-300"}
                      `}
                    >
                      {label}
                    </span>
                    {i < cycle.node_labels.length - 1 && (
                      <span className={`text-sm font-bold px-1 ${isDark ? "text-slate-500" : "text-gray-400"}`}>→</span>
                    )}
                  </React.Fragment>
                ))}
                <span className={`text-sm font-bold px-1 ${isDark ? "text-slate-500" : "text-gray-400"}`}>↩</span>
              ...
              `}>
                ...
                <div className="flex flex-col justify-center pl-2">
                  <div className={`text-xs uppercase tracking-wide font-semibold ${isDark ? "text-slate-500" : "text-gray-400"}`}>
                    Per Lap
                  </div>
                  <div className={`text-sm mt-1 font-bold ${isDark ? "text-slate-200" : "text-gray-800"}`}>
                    {(cycle.tokens_per_lap ?? 0).toLocaleString()} tokens
                  </div>
                  <div className={`text-xs font-mono font-medium mt-0.5 ${isDark ? "text-slate-400" : "text-gray-500"}`}>
                    ${(cycle.cost_per_lap ?? 0).toFixed(6)} · {((cycle.latency_per_lap ?? 0) * 1000).toFixed(1)} ms
                  </div>
                </div>

                {/* Contribution to total */}
                <div className="border-l pl-4 border-dashed border-gray-200 dark:border-slate-700">
                  <div className={`text-xs uppercase tracking-wide font-semibold ${isDark ? "text-slate-500" : "text-gray-400"}`}>
                    % of Total (avg)
                  </div>
                  <div className="flex items-center gap-4 mt-2">
                    <div className="text-center">
                      <div className={`text-[10px] font-medium ${isDark ? "text-slate-400" : "text-gray-500"}`}>Cost</div>
                      <div className={`text-sm font-bold tabular-nums ${costPct >= 50
                    ...
                    <div className="text-center">
                      <div className={`text-[10px] font-medium ${isDark ? "text-slate-400" : "text-gray-500"}`}>Latency</div>
                      <div className={`text-sm font-bold tabular-nums ${latencyPct >= 50
                          ? isDark ? "text-red-400" : "text-red-600"
                          : latencyPct >= 25
                            ? isDark ? "text-yellow-400" : "text-yellow-600"
                            : isDark ? "text-slate-300" : "text-gray-600"
