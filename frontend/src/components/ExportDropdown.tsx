...
function buildMarkdownReport(est: WorkflowEstimation): string {
  ...
  lines.push("# Neurovn — Workflow Estimation Report");
  ...
  lines.push("---");
  lines.push("*Report generated by Neurovn*\n");
  return lines.join("\n");
}
...
// ── Neurovn JSON Exporter ──────────────────────────────

function buildNeurovnExport(
  nodes: { id: string; type?: string; data: Record<string, unknown> }[],
  edges: { id: string; source: string; target: string }[],
...
async function generatePdfReport(
  ...
  doc.text("Neurovn", pageWidth / 2, y, { align: "center" });
  y += 7;
  doc.setFontSize(11);
...
export default function ExportDropdown({ isDark }: Props) {
  ...
  const exportPNG = useCallback(async () => {
    ...
    try {
      ...
      downloadDataUrl(dataUrl, "neurovn-workflow.png");
    } catch (err) {
  ...
  const exportSVG = useCallback(async () => {
    ...
    try {
      ...
      downloadDataUrl(dataUrl, "neurovn-workflow.svg");
    } catch (err) {
  ...
  const exportJSON = useCallback(() => {
    ...
    downloadBlob(json, "neurovn-report.json", "application/json");
    setOpen(false);
  ...
  const exportMarkdown = useCallback(() => {
    ...
    downloadBlob(md, "neurovn-report.md", "text/markdown");
    setOpen(false);
  }, [estimation]);

  const exportNeurovn = useCallback(() => {
    const json = buildNeurovnExport(nodes, edges, 50); // Defaulting recursion_limit to 50
    downloadBlob(json, "workflow.neurovn.json", "application/json");
    setOpen(false);
  }, [nodes, edges]);
  ...
  const exportDashboardPNG = useCallback(async () => {
    ...
    try {
      ...
      downloadDataUrl(dataUrl, "neurovn-dashboard.png");
    } catch (err) {
  ...
  const exportCSV = useCallback(() => {
    ...
    downloadBlob(csv, "neurovn-breakdown.csv", "text/csv");
    setOpen(false);
  }, [estimation]);
